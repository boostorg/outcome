...
BOOST_OUTCOME_REQUIRES(detail::type_can_be_used_in_result<P> && (std::is_void<P>::value || std::is_default_constructible<P>::value))      //
class outcome;
...
public:
  ...
  */
  BOOST_OUTCOME_TEMPLATE(class T)
  BOOST_OUTCOME_TREQUIRES(BOOST_OUTCOME_TPRED(convert::value_or_error<outcome, std::decay_t<T>>::enable_result_inputs || !is_result_v<T>),    //
                    BOOST_OUTCOME_TPRED(convert::value_or_error<outcome, std::decay_t<T>>::enable_outcome_inputs || !is_outcome_v<T>),  //
                    BOOST_OUTCOME_TEXPR(convert::value_or_error<outcome, std::decay_t<T>>{}(std::declval<T>())))
...
#pragma warning(pop)
  ...
  */
  failure_type<error_type, exception_type> as_failure() const &
  {
    ...
    return BOOST_OUTCOME_V2_NAMESPACE::failure(this->assume_error(), exception_type());
  }
  ...
  */
  failure_type<error_type, exception_type> as_failure() &&
  {
    ...
    return BOOST_OUTCOME_V2_NAMESPACE::failure(std::move(this->assume_error()), exception_type());
  }
};
