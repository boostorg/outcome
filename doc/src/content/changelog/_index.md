...
For those of you using `BOOST_OUTCOME_CO_TRY` within Coroutines, be aware that `decltype(co_await)` is not permitted
...
need to force the type of the internal unique to avoid the call to `decltype()` e.g. `BOOST_OUTCOME_CO_TRY((auto &&, v), co_await expr)`.
